...
for asset in assets:
    print(f"\n🔼 Upload {asset}.xlsx (must include '1min' and '5min' sheets):")
...
def run_stationarity_tests(series, name="Series"):
    ...
    print("→ ADF:", "Rejects H0 (stationary)" if adf_result[1] < 0.05 else "Fails to reject H0 (non-stationary)")
    print(f"KPSS Statistic = {kpss_result:.4f}, p-value = {kpss_p:.4f}")
    print("→ KPSS:", "Rejects H0 (non-stationary)" if kpss_p < 0.05 else "Fails to reject H0 (stationary)")

# STEP 3: Modeling and Forecasting (with printed predictions)
def model_and_forecast(series, name="Series"):
    print(f"\n============================")
    print(f"📈 Modeling for {name}")
    print(f"============================")

    # AR Model
    ar_model = AutoReg(series, lags=5).fit()
    ar_pred = ar_model.predict(start=len(series), end=len(series)+9)
    print(f"\n🔸 AR Forecast - {name}: {np.round(ar_pred.values, 2)}")
    ...
    print(f"\n🔸 ARMA Forecast - {name}: {np.round(arma_pred.values, 2)}")
    ...
    print(f"\n🔸 ARIMA Forecast - {name}: {np.round(arima_pred.values, 2)}")
...
def run_johansen_cointegration(data_dict):
    ...
    def try_test(df, label):
        try:
            ...
            print(f"✅ Johansen test successful for {label}")
...
print("\n📊 Engle-Granger Cointegration Test (2-step method):")
...
for x, y in pairs:
    try:
        ...
        print(f"Pair: {x} & {y} → p-value = {pval:.4f}")
        if pval < 0.05:
            print(f"✅ {x} and {y} are cointegrated (p < 0.05)")
        else:
            print(f"❌ No cointegration found for {x} and {y}")
    except Exception as e:
        print(f"⚠️ Error testing pair {x} & {y}: {str(e)}")