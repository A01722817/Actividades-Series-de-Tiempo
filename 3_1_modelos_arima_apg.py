# -*- coding: utf-8 -*-
"""3.1 Modelos ARIMA_APG

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/14-4j2mRzhKyxXtZSxYpL8wQ10RGWt7au
"""
...
def check_and_clean_series(series, name):
    print(f"\nChecking {name} for NaN or inf values:")
    if series.isna().any():
        print(f"  - Found {series.isna().sum()} NaN values in {name}")
    if np.isinf(series).any():
        print(f"  - Found inf values in {name}")

    # Remove NaN and inf values
    cleaned_series = series.replace([np.inf, -np.inf], np.nan).dropna()
    print(f"  - {name} cleaned: {len(series) - len(cleaned_series)} rows removed")
    return cleaned_series

tesla_close = check_and_clean_series(tesla_close, "Tesla")
nvidia_close = check_and_clean_series(nvidia_close, "NVIDIA")
meta_close = check_and_clean_series(meta_close, "Meta")
